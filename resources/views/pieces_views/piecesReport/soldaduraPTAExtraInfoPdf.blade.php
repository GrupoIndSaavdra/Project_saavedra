...
<head>
    <title>Reporte Soldadura PTA - Información Extra</title>
...
<body>
    <div class="header">
        <h2>Reporte de Información Extra - Soldadura PTA</h2>
        <p>Fecha de generación: {{ date('d-m-Y H:i') }}</p>
        <p>OT: <strong>{{ $ordenTrabajo ?? 'Todas' }}</strong></p>
        ...
        <tbody>
            @foreach ($piecesData as $piece)
                <tr>
                    ...
                    <td>{{ $piece['fecha'] }}</td>
                    <td>{{ $piece['hora'] }}</td>
                    <td>{{ $piece['precalentamiento'] }}</td>
                    <td>{{ $piece['material_soldadura'] ?? 'N/A' }}</td>
                    <td>{{ $piece['resultado'] }}</td>
                    <td>{{ $piece['defecto'] }}</td>
