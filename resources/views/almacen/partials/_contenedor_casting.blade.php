...
@php
    ...
    @php
        $filtrarArchivosCasting = function($archivos, $clasesPermitidas) {
            ...
            foreach ($archivos as $archivo) {
                ...
                foreach ($clasesPermitidas as $cp) {
                    ...
                    if ($claseEnArchivo === $cp || strpos($claseEnArchivo, $cp) !== false || 
        ...
        foreach ($dibujosCasting as $a) if (!in_array($a['nombre'], $idsDib)) $dibujosCastingPendientes[] = $a;

        $idsAyu = array_merge(array_column($ayudasCastingPendientes, 'nombre'), array_column($ayudasCastingProcesadas, 'nombre'));
        foreach ($ayudasCasting as $a) if (!in_array($a['nombre'], $idsAyu)) $ayudasCastingPendientes[] = $a;

        $idsLdm = array_merge(array_column($ldmCastingPendientes, 'nombre'), array_column($ldmCastingProcesados, 'nombre'));
        foreach ($calidadAprobadosLdmCasting as $a) if (!in_array($a['nombre'], $idsLdm)) $ldmCastingPendientes[] = $a;

        $idsPo = array_merge(array_column($preordenesCastingPendientes, 'nombre'), array_column($preordenesCastingProcesadas, 'nombre'));
        foreach ($almacenPreordenesCastingBase as $a) if (!in_array($a['nombre'], $idsPo)) $preordenesCastingPendientes[] = $a;
    ...
    <div class="alm-process-block" style="margin-bottom: 25px; padding: 20px; border-radius: 14px; background-color: #f0fdf4; border: 2px solid #16a34a; box-shadow: 0 4px 14px rgba(22, 163, 74, 0.08);">
        ...
        @if(count($aprobadosPendientesCasting) > 0 || (count($aprobadosPendientesCasting) == 0 && count($clasesAprobadasCubiertas) == 0))
            <div class="cal-subcontainer-almacen" style="margin-bottom: 25px; padding: 18px; border-radius: 12px; background-color: #f0fdf4; border: 2px solid #16a34a; box-shadow: 0 3px 10px rgba(22, 163, 74, 0.08);">
                ...
                @if (count($dibujosCastingPendientes) > 0)
                    ...
                    <div class="alm-pdf-grid" style="background-color: #dcfce7; border: 1px solid #bbf7d0; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                ...
                @if (count($ayudasCastingPendientes) > 0)
                    ...
                    <div class="alm-pdf-grid" style="background-color: #dcfce7; border: 1px solid #bbf7d0; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                        @foreach ($ayudasCastingPendientes as $archivoInfo)
                            ...
                            <div class="dibujos-file-card card-ayuda" style="animation-delay: {{ $loop->index * 0.05 }}s; border-left-color: #16a34a;">
                                <div class="file-icon-wrapper alm-cursor-pointer" title="Abrir PDF">
                                    <img src="{{ asset('images/pdf-view-shadow.png') }}" class="file-icon icon-default">
                                    <img src="{{ asset('images/pdf-view.png') }}" class="file-icon icon-hover">
                                </div>
                    ...
                    <div class="lib-calidad-card-body">
                        <div class="lib-calidad-action-row">
                            <h4 class="lib-calidad-card-prompt">
                                @if ($hasCastingPre)
                                    Pre-orden de casting generada {!! count($aprobados) > 0 ? 'para los modelos: <strong>' . e(implode(', ', array_map('ucfirst', $aprobados))) . '</strong>' : '' !!}. Puedes editar los datos o enviar la pre-orden por correo.
                                @elseif ($hasLdmSubidoCasting)
                                    Formatos LDM subidos {!! count($aprobados) > 0 ? 'para los modelos: <strong>' . e(implode(', ', array_map('ucfirst', $aprobados))) . '</strong>' : '' !!}. Procede a generar la Pre-Orden de Fabricación de Casting (PFC).
                                @else
                                    Modelos Aprobados por Calidad{!! !empty($aprobadosPendientesCasting) ? ': <strong>' . e(implode(', ', array_map('ucfirst', $aprobadosPendientesCasting))) . '</strong>' : (count($aprobados) > 0 ? ': <strong>' . e(implode(', ', array_map('ucfirst', $aprobados))) . '</strong>' : '') !!}. Procede a subir los formatos F-CCL-LDM firmados para iniciar el casting.
                                @endif
                            </h4>
                            
                            <div class="lib-calidad-card-btns">
                                {{-- Paso 1: Subir LDMs (solo cuando aún no se han subido por Almacén para las clases aprobadas y no hay pre-orden) --}}
                                ...
                                {{-- Paso 2: Generar Pre-Orden PFC (solo cuando los LDMs ya se subieron en Almacén y no hay pre-orden) --}}
        ...
        @if (count($clasesAprobadasCubiertas) > 0)
            <div class="cal-subcontainer-almacen" style="margin-bottom: 25px; padding: 18px; border-radius: 12px; background-color: #dcfce7; border: 2px solid #16a34a; box-shadow: 0 3px 10px rgba(22, 163, 74, 0.08);">
                <div style="display: flex; align-items: center; justify-content: space-between; border-bottom: 1.5px solid #bbf7d0; padding-bottom: 8px; margin-bottom: 15px;">
                    <h4 style="margin: 0; color: #15803d; font-size: 1.05rem; font-weight: 700; display: flex; align-items: center; gap: 8px;">
                ...
                @if (count($dibujosCastingProcesados) > 0)
                    ...
                    <div class="alm-pdf-grid" style="background-color: #dcfce7; border: 1px solid #bbf7d0; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                        @foreach ($dibujosCastingProcesados as $archivoInfo)
                            <div class="dibujos-file-card" style="animation-delay: {{ $loop->index * 0.05 }}s; border-left-color: #15803d;">
                                <div class="file-icon-wrapper alm-cursor-pointer" title="Abrir PDF">
                                    <img src="{{ asset('images/pdf-view-shadow.png') }}" class="file-icon icon-default">
                                    <img src="{{ asset('images/pdf-view.png') }}" class="file-icon icon-hover">
                                </div>
                ...
                @if (count($ayudasCastingProcesadas) > 0)
                    ...
                    <div class="alm-pdf-grid" style="background-color: #dcfce7; border: 1px solid #bbf7d0; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                        @foreach ($ayudasCastingProcesadas as $archivoInfo)
                            @php $ayudaUrl = $archivoInfo['url'] ?? ''; @endphp
                            <div class="dibujos-file-card card-ayuda" style="animation-delay: {{ $loop->index * 0.05 }}s; border-left-color: #15803d;">
                                <div class="file-icon-wrapper alm-cursor-pointer" title="Abrir PDF">
                                    <img src="{{ asset('images/pdf-view-shadow.png') }}" class="file-icon icon-default">
                                    <img src="{{ asset('images/pdf-view.png') }}" class="file-icon icon-hover">
                                </div>
                ...
                @if (count($ldmCastingProcesados) > 0)
                    ...
                    <div class="alm-pdf-grid" style="background-color: #dcfce7; border: 1px solid #bbf7d0; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                        @foreach ($ldmCastingProcesados as $otroArchivo)
                            <div class="dibujos-file-card card-otro" style="animation-delay: {{ $loop->index * 0.05 }}s; border-left-color: #15803d;">
                                <div class="file-icon-wrapper alm-cursor-pointer" title="Abrir PDF">
                                    <img src="{{ asset('images/pdf-view-shadow.png') }}" class="file-icon icon-default">
                                    <img src="{{ asset('images/pdf-view.png') }}" class="file-icon icon-hover">
                                </div>
                                ...
                                <div class="file-actions alm-flex-gap-5">
                                    <button class="btn-dibujos btn-dibujos-sm btn-ver alm-background-color-15803d alm-color-white" onclick="almacenVerPdf('{{ $otroArchivo['ot'] }}', '{{ $otroArchivo['nombre'] }}', '{{ $otroArchivo['tipo'] }}')">Ver</button>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="lib-calidad-card" style="margin-top: 20px; border: 2px solid #16a34a; border-radius: 12px; overflow: hidden;">
                    <div class="lib-calidad-card-header" style="background: #16a34a; display: flex; align-items: center; gap: 15px; padding: 12px 20px;">
                        <img src="{{ asset('images/Aprobado.png') }}" alt="Casting" style="width: 38px; height: 38px; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.2));">
                        <div class="alm-overflow-hidden alm-flex-1">
                            <span class="lib-calidad-card-title" style="color: white; font-weight: 700; font-size: 1.1rem; display: block;">Información — Casting</span>
                            <span class="lib-calidad-card-ot" style="color: #dcfce7; font-size: 0.85rem;">{{ preg_replace('/_\\d{8}_\\d{6}_.*/', '', $reg->ot) }}</span>
                        </div>
                    </div>
                    <div class="lib-calidad-card-body" style="background: #f0fdf4; padding: 20px;">
                        <div class="lib-calidad-action-row" style="display: flex; align-items: center; gap: 20px;">
                            ...
                            <div>
                                <h4 class="lib-calidad-card-prompt" style="color: #15803d; margin-top: 0; margin-bottom: 8px; font-weight: 700; font-size: 1.1rem;">
