    ...
    @if ($tieneFabricacion)
        @php
            ...
            $esReinicioParcial =
                $isCalidadAlerted && count($clasesFabricacion) > 0 && !$esReproceso && $hasRechazosReal;
            ...
            if ($esReinicioParcial) {
                ...
                $otClasesActivas = !empty($clasesFabricacion)
                    ...
                    : (is_array($reg->ayudas_config)
                        ? array_map('strtolower', $reg->ayudas_config)
                        : []);
            }

            $preOrdenesFabExistentes = \App\Models\PreOrdenFundicion::where('ot', $targetReg->ot)
                ->get()
                ->filter(function ($po) use ($clasesFabricacion) {
                    if (
                        $po->pdf_filename &&
                        (str_contains($po->pdf_filename, '_Anterior_N') ||
                            str_contains($po->pdf_filename, 'Casting') ||
                            str_contains($po->pdf_filename, 'F_ALM_PFC_') ||
                            str_contains($po->pdf_filename, 'PFC'))
                    ) {
                        return false;
                    }
                    ...
                    foreach ($filas as $f) {
                        $c = strtolower(
                            $f['clase'] ?? ($f['clase_nombre'] ?? ($f['tipo_modelo'] ?? ($f['nombre'] ?? ''))),
                        );
                        foreach ($clasesFabricacion as $cf) {
                            if (
                                $c !== '' &&
                                ($c === strtolower($cf) ||
                                    strpos($c, strtolower($cf)) !== false ||
                                    strpos(strtolower($cf), $c) !== false)
                            ) {
                                return true;
                            }
                        }
                    ...
                    ->filter(function ($lib) use ($clasesFabricacion) {
                        ...
                        foreach ($clasesFabricacion as $cf) {
                            if (
                                $tm !== '' &&
                                ($tm === strtolower($cf) ||
                                    strpos($tm, strtolower($cf)) !== false ||
                                    strpos(strtolower($cf), $tm) !== false)
                            ) {
                                return true;
                            }
                        }
                        ...
                        if (!empty($cVal)) {
                            ...
                            foreach ($clasesFabricacion as $cf) {
                                if (
                                    !empty($cf) &&
                                    (strtolower($cf) === $cVal ||
                                        strpos($cVal, strtolower($cf)) !== false ||
                                        strpos(strtolower($cf), $cVal) !== false)
                                ) {
                                    $inFab = true;
                                    break;
                                }
                    ...
                    foreach (explode(',', $lf) as $c) {
                        ...
                        foreach ($clasesFabricacion as $cf) {
                            if (
                                !empty($cf) &&
                                (strtolower($cf) === $cTrim ||
                                    strpos($cTrim, strtolower($cf)) !== false ||
                                    strpos(strtolower($cf), $cTrim) !== false)
                            ) {
                                $inFab = true;
                                break;
                            }
            ...
            $faltantesEnPreOrdenPendiente = false;
            if ($poPendienteEnvioFab !== null && count($clasesActivasFaltantes) > 0) {
                $filasPoPendiente = is_string($poPendienteEnvioFab->filas)
                    ? json_decode($poPendienteEnvioFab->filas, true)
                    : $poPendienteEnvioFab->filas;
                $clasesPoPendiente = [];
                if (is_array($filasPoPendiente)) {
                    foreach ($filasPoPendiente as $f) {
                        $c = strtolower(
                            trim($f['clase'] ?? ($f['clase_nombre'] ?? ($f['tipo_modelo'] ?? ($f['nombre'] ?? '')))),
                        );
                        $c = trim(preg_replace('/^modelo\s+/i', '', $c));
                        if (!empty($c)) {
                            $clasesPoPendiente[] = preg_replace('/[^a-zA-Z0-9]/', '', $c);
                        }
                    }
                }
                $todasCubiertas = true;
                foreach ($clasesActivasFaltantes as $caf) {
                    $cafClean = strtolower(preg_replace('/[^a-zA-Z0-9]/', '', $caf));
                    $cubierta = false;
                    foreach ($clasesPoPendiente as $cpp) {
                        if (strpos($cpp, $cafClean) !== false || strpos($cafClean, $cpp) !== false) {
                            $cubierta = true;
                            break;
                        }
                    }
            ...
            $controlDisabled =
                count($clasesFabricacion) > 0 || $esReinicioParcial
                    ? ''
                    : ($targetReg->tiene_modelo || $targetReg->pre_orden_sent || $targetReg->pre_orden_email_sent
            ...
            $hideTengoModelo =
                $todasClasesProcesadas || $targetReg->tiene_modelo || $faltantesEnPreOrdenPendiente
                    ...
                    : '';
            $hideGenerarFormato =
                $todasClasesProcesadas || $poPendienteEnvioFab !== null || $esReproceso ? 'display: none;' : '';
                ...
                count($clasesFabricacion) > 0
                    ...
                    : $calidadYaRespondio ||
                        (($tieneAprobados || $tieneRechazados) &&
                            (!$esReproceso || count($clasesFabricacion) === 0 || !$hasRechazosReal));
        @endphp
                ...
                <div class="cal-subcontainer-almacen"
                    ...
                    <div
                        ...
                        <h4
                            ...
                            Procesos Activos ({{ implode(', ', array_map($formatClase, $clasesActivasFaltantes)) }})
                        </h4>
                    </div>

                    {{-- Dibujos de Modelo Pendientes --}}
                    @if (count($dibujosPendientes) > 0)
                        <h4 style="margin-top: 10px; margin-bottom: 10px; color: #005194; font-weight: 700;">Dibujos /
                            DWG de Fundición
                        </h4>
                    ...
                    @if (count($ayudasPendientes) > 0)
                        <h4 style="margin-top: 15px; margin-bottom: 10px; color: #9c0300; font-weight: 700;">Ayudas
                            Visuales de
                            Fundición</h4>
                        <div class="alm-pdf-grid"
                            style="background-color: #e0f2fe; border: 1px solid #bae6fd; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                    ...
                    @if ($showControlCard && !$ocultarCardEnModelo)
                        @php
                            ...
                            foreach ($enviadas as $envClass => $hash) {
                                ...
                                $clasesAlertadas[] = str_replace(
                                    [' ', '_', 'í', 'gú'],
                                    ['', '', 'i', 'gu'],
                                    strtolower(trim($claseNombre)),
                                );
                            }
                            ...
                            <div class="lib-calidad-card-body">
                                <div class="lib-calidad-action-row">
                                    <h4 class="lib-calidad-card-prompt">
                                        @if (!$hasAlertedClass)
                                            <span class="alm-color-dc2626 alm-font-weight-700"
                                                style="color: #dc2626; font-weight: 700;">
                                                Bloqueado: Esperando a que <span
                                                    style="background: #fef2f2; color: #b91c1c; padding: 3px 8px; border-radius: 6px; font-weight: 700; font-size: 0.95em; border: 1px solid #fca5a5; display: inline-block; margin: 2px 0;">Ingeniería
                                                    (Dibujos de Fundición)</span> envíe la notificación a
                                                <span
                                                    style="background: #fef2f2; color: #b91c1c; padding: 3px 8px; border-radius: 6px; font-weight: 700; font-size: 0.95em; border: 1px solid #fca5a5; display: inline-block; margin: 2px 0;">Almacén</span>.
                                            </span>
                                        ...
                                        @else
                                            El departamento de <span
                                                style="background: #fdf4ff; color: #a21caf; padding: 3px 8px; border-radius: 6px; font-weight: 700; font-size: 0.95em; border: 1px solid #f0abfc; display: inline-block; margin: 2px 0;">Programación</span>
                                            te envió los <span
                                                style="background: #f0f9ff; color: #0369a1; padding: 3px 8px; border-radius: 6px; font-weight: 700; font-size: 0.95em; border: 1px solid #7dd3fc; display: inline-block; margin: 2px 0;">Dibujos
                                                y Ayudas Visuales de Fundición</span> correspondientes a la OT. ¿Cuentas
                                            con el
                                            modelo en físico para hacer la <span
                                                ...
                                                de Fabricación de Modelo (F_ALM_PFM)</span>?
                                        @endif
                                    </h4>
            ...
            @if (count($clasesActivasCubiertas) > 0)
                <div class="cal-subcontainer-almacen"
                    ...
                    <div
                        ...
                        <h4
                            ...
                            <img src="{{ asset('images/Aprobado.png') }}"
                                style="width: 22px; height: 22px; object-fit: contain;">
                            Clases Procesadas ({{ implode(', ', array_map($formatClase, $clasesActivasCubiertas)) }})
                        </h4>
                        <span
                    ...
                    @if (count($dibujosProcesados) > 0)
                        <h4 style="margin-top: 10px; margin-bottom: 10px; color: #0369a1; font-weight: 700;">Dibujos /
                            DWG de Fundición
                        </h4>
                        <div class="alm-pdf-grid"
                            style="background-color: #e0f2fe; border: 1px solid #bae6fd; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                    ...
                    @if (count($ayudasProcesadas) > 0)
                        <h4 style="margin-top: 15px; margin-bottom: 10px; color: #0369a1; font-weight: 700;">Ayudas
                            Visuales de
                            Fundición</h4>
                        <div class="alm-pdf-grid"
                            style="background-color: #e0f2fe; border: 1px solid #bae6fd; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                    ...
                    @if (count($preordenesProcesadas) > 0)
                        <h4 style="margin-top: 15px; margin-bottom: 10px; color: #0369a1; font-weight: 700;">
                            Pre-órdenes/Confirmaciones
                            de Modelo</h4>
                        <div class="alm-pdf-grid"
                            style="background-color: #e0f2fe; border: 1px solid #bae6fd; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                            ...
                            <div class="lib-calidad-action-row"
                                ...
                                <div>
                                    ...
                                    <p style="color: #0c4a6e; margin: 0; font-size: 0.95rem; font-weight: 500;">
                                        El proceso ahora le pertenece a Calidad. Por favor, espera instrucciones para
                                        las clases
                                        enviadas.
                                    </p>
                                </div>
