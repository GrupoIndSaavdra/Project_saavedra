...
@php
                                                                            ...
                                                                            $hideGenerarFormato = ($todasClasesProcesadas || $poPendienteEnvioFab !== null) ? 'display: none;' : '';
                                                                            $hideReprocesoPreOrden = 'display: none;';
    ...
    @php
        ...
        $idsAsignados = array_merge(
            array_column($dibujosPendientes, 'nombre'), array_column($dibujosProcesados, 'nombre')
        );
        foreach ($dibujosModelo as $a) {
            if (!in_array($a['nombre'], $idsAsignados)) $dibujosPendientes[] = $a;
        }
        
        $idsAsignadosAyu = array_merge(
            array_column($ayudasPendientes, 'nombre'), array_column($ayudasProcesadas, 'nombre')
        );
        foreach ($ayudasModelo as $a) {
            if (!in_array($a['nombre'], $idsAsignadosAyu)) $ayudasPendientes[] = $a;
        }
        
        $idsAsignadosPo = array_merge(
            array_column($preordenesPendientes, 'nombre'), array_column($preordenesProcesadas, 'nombre')
        );
        foreach ($almacenPreordenesFab as $a) {
            if (!in_array($a['nombre'], $idsAsignadosPo)) $preordenesPendientes[] = $a;
        }
    @endphp
    ...
    <div class="alm-process-block" style="margin-bottom: 25px; padding: 20px; border-radius: 14px; background-color: #f0f9ff; border: 2px solid #0284c7; box-shadow: 0 4px 14px rgba(2, 132, 199, 0.08);">
        ...
        @if(count($clasesActivasFaltantes) > 0 || (count($clasesActivasFaltantes) == 0 && count($clasesActivasCubiertas) == 0))
            <div class="cal-subcontainer-almacen" style="margin-bottom: 25px; padding: 18px; border-radius: 12px; background-color: #f0f9ff; border: 2px solid #0ea5e9; box-shadow: 0 3px 10px rgba(14, 165, 233, 0.08);">
                ...
                @if ($showControlCard && !$ocultarCardEnModelo)
                    <div class="lib-calidad-card" id="control-modelo-{{ md5($reg->ot) }}" style="margin-top: 20px; {{ trim($controlDisabled . ' ' . $hideControlCard) }}">
                        ...
                        <div class="lib-calidad-card-body">
                            <div class="lib-calidad-action-row">
                                <h4 class="lib-calidad-card-prompt">
                                    @if ($algunaClaseProcesada && !$todasClasesProcesadas)
                                    ...
                                    @elseif($targetReg->pre_orden_email_sent && !$esReinicioParcial)
                                        Pre-orden enviada por correo al proveedor. Esperando entrega de modelo físico para revisión de Calidad.
                                    ...
                                    @else
                                        Formatos LDM subidos. ¿Cuentas con el modelo en físico para la etapa de <strong>Fabricación</strong>?
                                    @endif
                                </h4>
                                <div class="lib-calidad-card-btns" style="{{ $hideAllBtns }}">
                                    @if((!$targetReg->tiene_modelo || $esReinicioParcial) && !$esReproceso)
        ...
        @if (count($clasesActivasCubiertas) > 0)
            <div class="cal-subcontainer-almacen" style="margin-bottom: 25px; padding: 18px; border-radius: 12px; background-color: #e0f2fe; border: 2px solid #0284c7; box-shadow: 0 3px 10px rgba(2, 132, 199, 0.08);">
                <div style="display: flex; align-items: center; justify-content: space-between; border-bottom: 1.5px solid #bae6fd; padding-bottom: 8px; margin-bottom: 15px;">
                    <h4 style="margin: 0; color: #0369a1; font-size: 1.05rem; font-weight: 700; display: flex; align-items: center; gap: 8px;">
                    ...
                    <span style="font-size: 0.75rem; font-weight: 700; background: #dcfce7; color: #0369a1; padding: 3px 10px; border-radius: 6px; border: 1px solid #86efac;">
                        ENVIADO A CALIDAD
                    </span>
                </div>
                ...
                @if (count($dibujosProcesados) > 0)
                    <h4 style="margin-top: 10px; margin-bottom: 10px; color: #0369a1; font-weight: 700;">Dibujos de Fundición (Modelo)</h4>
                    <div class="alm-pdf-grid" style="background-color: #e0f2fe; border: 1px solid #bae6fd; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                ...
                @if (count($ayudasProcesadas) > 0)
                    <h4 style="margin-top: 15px; margin-bottom: 10px; color: #0369a1; font-weight: 700;">Ayudas Visuales de Fundición (Modelo)</h4>
                    <div class="alm-pdf-grid" style="background-color: #e0f2fe; border: 1px solid #bae6fd; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                ...
                @if (count($preordenesProcesadas) > 0)
                    <h4 style="margin-top: 15px; margin-bottom: 10px; color: #0369a1; font-weight: 700;">Documentos / Pre-órdenes de Fabricación</h4>
                    <div class="alm-pdf-grid" style="background-color: #e0f2fe; border: 1px solid #bae6fd; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                ...
                <div class="lib-calidad-card" style="margin-top: 20px; border: 2px solid #0284c7; border-radius: 12px; overflow: hidden;">
                    <div class="lib-calidad-card-header" style="background: #0284c7; display: flex; align-items: center; gap: 15px; padding: 12px 20px;">
                        <img src="{{ asset('images/almacen.png') }}" alt="Almacén" style="width: 38px; height: 38px; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.2));">
                        <div class="alm-overflow-hidden alm-flex-1">
                            <span class="lib-calidad-card-title" style="color: white; font-weight: 700; font-size: 1.1rem; display: block;">Información — Almacén (Fabricación)</span>
                            <span class="lib-calidad-card-ot" style="color: #bae6fd; font-size: 0.85rem;">{{ preg_replace('/_\d{8}_\d{6}_.*/', '', $reg->ot) }}</span>
                        </div>
                    </div>
                    <div class="lib-calidad-card-body" style="background: #f0f9ff; padding: 20px;">
                        <div class="lib-calidad-action-row" style="display: flex; align-items: center; gap: 20px;">
                            ...
                            <div>
                                <h4 class="lib-calidad-card-prompt" style="color: #0369a1; margin-top: 0; margin-bottom: 8px; font-weight: 700; font-size: 1.1rem;">
