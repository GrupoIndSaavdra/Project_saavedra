...
<body>
    ...
    <table class="tbl-header">
        <tr>
            ...
            <td class="title-cell">
                PREORDEN DE FABRICACIÓN DE MODELOS
            </td>
            <td class="logo-cell">
    ...
    <table class="tbl-general">
        <tr>
            ...
            <td class="gen-value" style="width: 14%;">
                {{ \Carbon\Carbon::parse($data['fecha_creacion'])->format('d/m/Y') }}</td>
        ...
        <tbody>
            @foreach($data['filas'] as $index => $fila)
                <tr class="{{ $index % 2 == 1 ? 'row-alt' : '' }}">
                    ...
                    <td>
                        @if(stripos($fila['clase_nombre'], 'molde') !== false || stripos($fila['clase_nombre'], 'bombillo') !== false)
                            N/A
                        @else
                            {{ $fila['impresiones'] }}
                        @endif
                    ...
                    @if($index === 0)
                        <td class="date-rowspan" rowspan="{{ count($data['filas']) }}"
    ...
    <div class="footer">
        Grupo Industrial Saavedra &nbsp;|&nbsp; Formato 4ALM-16 &nbsp;|&nbsp; Generado el {{ date('d/m/Y H:i') }}
        &nbsp;|&nbsp; Copia Controlada
    </div>
