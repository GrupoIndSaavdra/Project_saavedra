// ── CASTING APPROVED MATERIAL FILES RENDERING & ACTIONS ──

window.pintarCardsArchivosAprobados = function (archivos, ot, contenedorId, tipoModelo) {
    ...
    if (archivos && archivos.length > 0) {
        archivos.forEach((file) => {
            ...
            const isAprobadoFolder = fileNameLower.includes("documentos_aprobados") || fileNameLower.includes("aprobado");
            
            if (matchModelo && isAprobadoFolder) {
                count++;
                ...
                const fileUrl = `${window.baseUrl || "/"}${file.nombre}`;
                ...
                card.innerHTML = `
                    ...
                    <div class="file-actions" style="margin-top:auto; display:flex; gap:6px; width:100%;">
                        ...
                        <button type="button" class="btn-dibujos btn-dibujos-sm" style="flex:1; background:#dc2626; color:white; border:none; cursor:pointer;" onclick="window.eliminarArchivoAprobadoServer('${ot}', '${file.nombre}', '${tipoModelo}')">Eliminar</button>
                    </div>
                `;
    ...
    if (count > 0) {
        window.bloquearBotonEstatico(otClean, tipoModeloNorm, "aprobados");
    } else {
        window.desbloquearBotonEstatico(otClean, tipoModeloNorm, "aprobados");
        contenedor.innerHTML = `
            <div style="text-align: center; color: #64748b; padding: 15px; font-style: italic; width: 100%; font-family: 'Poppins', sans-serif;">
                No se ha subido ningún formato escaneado F-CCL-LDM para este modelo.
            </div>
        `;
    }
};

window.subirArchivoAprobadoEstatico = function (ot, tipoModelo, inputId) {
...
window.eliminarArchivoAprobadoServer = function (ot, fileNombre, tipoModelo) {
