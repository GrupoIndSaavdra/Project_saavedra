...
window.calidadVerPdf = function (ot, archivo, tipo = "dibujo") {
    ...
    const url = routesObj.serve + "?ot=" + encodeURIComponent(ot) + "&archivo=" + encodeURIComponent(archivo) + "&tipo=" + encodeURIComponent(tipo);
        ...
        .then((data) => {
            ...
            if (data.success) {
                ...
                const container =
                    ...
                    document.getElementById(`status-modelo-${ot}`) || document.getElementById(`status-modelo-${ot.replace(/_R\d+$/i, "")}`);
...
function buildPayload(tbodyId, formIds) {
    ...
    for (let i = 0; i < tbody.rows.length; i++) {
        ...
        const claseNombreFinal = rawClassName.startsWith(prefijo + " ") ? rawClassName : `${prefijo} ${rawClassName}`;
...
`;
    ...
    if (!hideRechazados) {
        ...
        sectionsHtml += makeCategorySection(
            ...
            inputNameMode === "calidad" ? "Documentos Rechazados" : "Documentos Rechazados (SCAR)",
                ...
                Array.from(this.files).forEach((file) => {
                    ...
                    if (!alreadyExists) {
                        ...
                        if (fnameLower.includes("ldm")) {
                            ...
                            else if (fnameLower.includes("obturador") && window.micRequiredClasses.includes("obturador"))
...
`;
        ...
        document.getElementById("lib-btn-accion")?.addEventListener("click", () => _libSubmit("accion"));
        ...
        if (data.success) {
            ...
            if (accion === "guardar") {
                ...
                const esRechazoPorDecision =
                    ...
                    document.getElementById("lib-accion")?.value === "rechazar" || (activeDecisionEl && activeDecisionEl.id === "lib-dec-rechazar");
...
const ModeloStateMachine = (() => {
    ...
    function _render(ot, estado, cfg) {
        ...
        if (!el) {
            ...
            console.warn(`[FSM] Contenedor no encontrado: #status-modelo-${ot}`);
...
`;
    ...
    function transicion(ot, estado) {
        ...
        if (!cfg) {
            ...
            console.warn(`[FSM] Estado desconocido: "${estado}" (canonical: "${canonical}")`);
    ...
    if (codigoInput) {
        ...
        if (tipoModelo) {
            ...
            if (tLow.includes("templadera")) {
                ...
                if (tLow === "bombillo" || tLow.includes("bombillo")) prefix = "B";

                else if (tLow === "obturador" || tLow.includes("obturador")) prefix = "O";

                else if (tLow === "molde" || tLow.includes("molde")) prefix = "M";

                else if (tLow === "fondo" || tLow.includes("fondo")) prefix = "F";

                else if (tLow.includes("cabeza") && tLow.includes("soplo")) prefix = "CS";
            ...
            if (data.success) {
                ...
                if (data.preorden_codigo_modelo) {
                    ...
                    if (esTempladera) {
                        ...
                        if (tLow === "bombillo" || cLow.includes("bombillo")) prefix = "B";
                            ...
                            prefix = (data.clase_nombre || tipoModelo || "F").charAt(0).toUpperCase();
                ...
                if (data.scar) {
                    ...
                    if (chkFotos) {
                        ...
                        if (group) group.classList.toggle("cal-display-none", !(chkFotos.checked));
                    ...
                    if (chkOtro) {
                        ...
                        if (group) group.classList.toggle("cal-display-none", !(chkOtro.checked));
                    ...
                    if (chkAccionOtro) {
                        ...
                        if (group) group.classList.toggle("cal-display-none", !(chkAccionOtro.checked));
...
function _libSetDecisionUI(decision) {
    ...
    if (decision === "aprobar") {
        ...
        if (bloqueRechazo) bloqueRechazo.classList.add("cal-display-none");
        ...
        if (bloqueRechazo) bloqueRechazo.classList.remove("cal-display-none");
...
window.abrirModalEnviarAlertaLiberacion = function (ot, decision, tiposAprobados, tiposRechazados, isAlmacen = false) {
    ...
    if (rowsA)
        ...
        rowsA.innerHTML = arrAprobados.length
            ...
            ? arrAprobados.map((t) => _crearFilaUpload(t, "#059669", "#f0fdf4", false, baseUrl)).join("")
    ...
    if (rowsR)
        ...
        rowsR.innerHTML = arrRechazados.length
            ...
            ? arrRechazados.map((t) => _crearFilaUpload(t, "#dc2626", "#fef2f2", true, baseUrl)).join("")
            ...
            if (data.existe && data.archivos?.length > 0) {
                ...
                const filteredFiles = data.archivos.filter((f) => {
                    ...
                    const isPreordenFile =
                        ...
                        pl.includes("pre-orden") || pl.includes("preorden") || (pl.includes("confirmacionmodelo") && !pl.includes("casting"));
...
`;
                            ...
                            if (prompt) prompt.classList.add("cal-display-none");
...
window.abrirModalPreOrdenCasting = async function (ot) {
    ...
    document.getElementById("btn-remove-poc-page-2").classList.add("cal-display-none");

    document.getElementById("btn-add-poc-page-2").classList.remove("cal-display-none");
            ...
            if (castingOrders.length > 0) {
                ...
                if (castingOrders.length > 1) {
                    ...
                    pocState.page2.fecha_entrega = po2.fecha_entrega
                        ...
                        : po2.filas[0]?.fecha_entrega || res.fecha_entrega || "";
                    ...
                    document.getElementById("tab-poc-page-2").classList.remove("cal-display-none");

                    document.getElementById("btn-add-poc-page-2").classList.add("cal-display-none");

                    document.getElementById("btn-remove-poc-page-2").classList.remove("cal-display-none");
...
window.switchPocPage = function (pageNum) {
    ...
    if (pageNum === 1) {
        ...
        document.getElementById("poc-page-1").classList.add("cal-display-none");

        document.getElementById("poc-page-2").classList.remove("cal-display-none");
        ...
        const archivosSeccion = archivos.filter((f) => {
            ...
            if (sec.tipos.includes("aprobado")) {
                ...
                return (
                    ...
                    tipo === "aprobado" || tipo === "preorden" || tipo === "otro" || nombre.includes(targetFolder) || nombre.includes("preordenes/")
...
`;
                        ...
                        if (downloadBtn) {
                            ...
                            if (data.casting_pdf_generated) {
                                ...
                                downloadBtn.classList.remove("cal-display-none");
            ...
            if (files && files.length > 0) {
                ...
                existingScar = files.find((f) => {
                    ...
                    return (
                        ...
                        nameLower.includes("documentos_rechazados/scar/") && filename.startsWith("scar_" + c.toLowerCase() + "_" + sanitizedOt + ".")
                        ...
                        if (filesContainer) {
                            ...
                            const baseRech = (data.archivos || []).filter((f) => {
                                ...
                                if (
                                    ...
                                    (nombre.includes("pre-orden") && nombre.includes("fundicion") && !nombre.includes("modelo"))
                            ...
                            const sectionsHtml = window.generarHtmlCategorizadoCastingAprobados

                                ? window.generarHtmlCategorizadoCastingAprobados(filtrados, otClean, true)
...
window.switchMgvTab = function (tabName) {
    ...
    document.querySelectorAll(".mgv-view").forEach((v) => v.classList.add("cal-display-none"));

    document.querySelectorAll(".mgv-tab").forEach((t) => t.classList.remove("active"));

    document.getElementById("mgv-view-" + tabName).classList.remove("cal-display-none");
...
window.abrirModalGestionVeredicto = function (ot, aprobados, rechazados) {
    ...
    if (tabAprobados) tabAprobados.classList.toggle("cal-display-none", !(hasAprobados));

    if (tabRechazados) tabRechazados.classList.toggle("cal-display-none", !(hasRechazados));
        ...
        .then((data) => {
            ...
            if (btnIr) {
                ...
                if (hasPreordenCasting && hasRechazados) {
                    ...
                    if (tabAprobados) tabAprobados.classList.add("cal-display-none");
            ...
            if (data.existe && data.archivos && data.archivos.length > 0) {
                ...
                if (filesContainerA) {
                    ...
                    filesContainerA.innerHTML =
                        ...
                        generarHtmlCategorizadoCastingAprobados(archivosAprob, otClean, false) ||
                ...
                const baseRech = (data.archivos || []).filter((f) => {
                    ...
                    if (nombre.includes("aprobado") || (nombre.includes("pre-orden") && nombre.includes("fundicion") && !nombre.includes("modelo")))
                ...
                if (filesContainerR) {
                    ...
                    filesContainerR.innerHTML =
                        ...
                        generarHtmlCategorizadoCastingAprobados(archivosRech, otClean, true) ||
        ...
        const p = fetch(`${routesObj.archivos}?ot=${encodeURIComponent(ot)}`)
            ...
            .then(data => {
                ...
                if (data.existe) {
                    ...
                    const count = Array.isArray(data.archivos) ? data.archivos.length : 0;
    ...
    Promise.all(promises).then(() => {
        ...
        if (!manual && cambiosDetectados > 0) {
            ...
            if (typeof toastpremium === 'function') {
                ...
                toastpremium(`Se detectaron cambios en archivos de ${cambiosDetectados} OT(s).`, 'warning');

            } else {

                almacenToast(`Se detectaron cambios en archivos de ${cambiosDetectados} OT(s).`, 'warning');
        ...
        if (!statusTime) statusTime = document.getElementById('sync-last-time-almacen');
