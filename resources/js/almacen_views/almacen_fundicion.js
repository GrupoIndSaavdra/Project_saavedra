...
window.abrirModalEnviarPreOrden = function (ot, tipo, clasesFaltantes = null) {
    ...
    const inputDestinatarioCalidad = document.getElementById(
        "env-destinatario-calidad",
    );

    const divDestinatarioCalidad = document.getElementById(
        "div-env-destinatario-calidad",
    );
    ...
    if (inputDestinatarioCalidad && divDestinatarioCalidad && form) {
        if (tipo === "casting") {
            ...
            inputDestinatarioCalidad.value =
                form.getAttribute("data-email-calidad");
        }
    }
    ...
    fetch(
        `${window.almacenRoutes.archivos}?ot=${encodeURIComponent(ot)}&tipo=${encodeURIComponent(tipo || "modelo")}`,
    )
        .then((res) => res.json())
            ...
            if (data.existe && data.archivos && data.archivos.length > 0) {
                ...
                if (
                    ...
                    fileCards.forEach((card) => {
                        ...
                        if (tipo === "casting") {
                            if (
                                (fileName.includes("pre-orden") ||
                                    fileName.includes("preorden")) &&
    ...
    const pendingContainer = document.getElementById(
        "env-pending-preordenes-container",
    );
    ...
    if (pendingContainer) {
        ...
        fetch(
            `${window.almacenRoutes.pendingPreOrdenes}?ot=${encodeURIComponent(ot)}&tipo=${encodeURIComponent(tipo || "modelo")}`,
        )
            .then((res) => res.json())
            ...
            .then((data) => {
                if (data.success && data.pending && data.pending.length > 0) {
                    ...
                    data.pending.forEach((po) => {
                        if (
                            po.proveedor &&
                            po.proveedor.toLowerCase().includes("jacarandas")
                        ) {
                            hasJacarandas = true;
                        }
                    ...
                    const inputDestinatario =
                        document.getElementById("env-destinatario");

                    if (hasJacarandas && form && inputDestinatario) {
                        const jEmail = form.getAttribute(
                            "data-email-jacarandas",
                        );
...
window.syncArchivosSeleccionadosPreOrden = function () {
    ...
    if (
        pendingContainer.querySelector(".alm-spinner") ||
        filesContainer.querySelector(".alm-spinner")
    )
        return;

    const preOrdenesChecked = Array.from(
        pendingContainer.querySelectorAll(
            'input[name="pre_orden_ids[]"]:checked',
        ),
    );
    ...
    const archivoChecks = filesContainer.querySelectorAll(
        'input[name="archivos_seleccionados[]"]',
    );
...
function renderMicAdicionalesBadges() {
    ...
    micAdicionalesSelectedFiles.forEach((item, index) => {
        ...
        row.classList.add("alm-row-base");
...
function renderAlFotosBadges() {
    ...
    if (alFotosSelectedFiles.length > 0) {
        ...
        alFotosSelectedFiles.forEach((file, index) => {
            ...
            badge.classList.add("alm-badge-base");
...
window.onCmClaseToggle = function (checkbox) {
    ...
    checkbox.parentElement.style.borderColor = checkbox.checked
        ? "#0a8504"
        : "#cbd5e1";

    checkbox.parentElement.style.backgroundColor = checkbox.checked
        ? "#f0fdf4"
        : "#fff";
    ...
    const fileCards = document.querySelectorAll(
        "#cm-server-files-container .select-file-card",
    );
...
window.abrirModalConfirmarModelo = function (
            ...
            todasClases.forEach((nombreClase, index) => {
                ...
                if (
                    ...
                    const esFaltante = clasesFaltantes.some(
                        (f) =>
                            f.toLowerCase().includes(nombreNorm) ||
                            nombreNorm.includes(f.toLowerCase()),
                    );
    ...
    if (filesContainer) {
        ...
        fetch(
            `${window.almacenRoutes.archivos}?ot=${encodeURIComponent(ot)}&tipo=modelo`,
        )
            .then((res) => res.json())
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        form.addEventListener("submit", async function (e) {
            ...
            const selectedClasses = document.querySelectorAll(
                ".cm-clase-checkbox:checked",
            );

            if (selectedClasses.length === 0) {
                almacenToast(
                    "Debes seleccionar al menos una clase para confirmar su modelo físico.",
                    "error",
                );
            ...
            const totalPending =
                document.querySelectorAll(".cm-clase-checkbox").length;

            const selectedPending = document.querySelectorAll(
                ".cm-clase-checkbox:checked",
            ).length;
            ...
            if (window.otCurrentClasses) {
                const yaUsadas = Array.from(
                    document.querySelectorAll(".po-clase-select"),
                )

                    .map(
                        (s) =>
                            s.options[s.selectedIndex]?.text?.toLowerCase() ||
                            "",
                    )
                ...
                const availableClasses = window.otCurrentClasses.filter((c) => {
                    ...
                    return !yaUsadas.some(
                        (yp) =>
                            nombreNorm.includes(yp) || yp.includes(nombreNorm),
                    );
                });
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (allConfirmed) {
                        ...
                        console.log(
                            "Confirmación parcial, recargando para reflejar cambios...",
                        );
                    }
                ...
                if (data.success) {
                    ...
                    buttons.forEach((b) => {
                        ...
                        if (card) {
                            ...
                            if (prompt)
                                prompt.classList.add("alm-display-none");
                        }
                    });
    ...
    try {
        ...
        if (res.success) {
            ...
            if (res.pre_ordenes && Array.isArray(res.pre_ordenes)) {
                castingPos = res.pre_ordenes.filter(
                    (po) =>
                        po.pdf_filename &&
                        po.pdf_filename.toLowerCase().includes("casting"),
                );
            }
            ...
            if (castingPos.length > 0) {
                ...
                const fillPageData = (pageObj, poRecord) => {
                    ...
                    pageObj.fecha_entrega = poRecord.fecha_entrega
                        ? poRecord.fecha_entrega.substring(0, 10)
                        : res.fecha_entrega || "";

                    pageObj.fecha = poRecord.fecha
                        ? poRecord.fecha.substring(0, 10)
                        : pageObj.fecha;
...
window.sincronizarDibujos = function (manual = false) {
        ...
        const p = fetch(`${routesObj.archivos}?ot=${encodeURIComponent(ot)}`)
            ...
            .then((data) => {
                if (data.existe) {
                    ...
                    // Si es polling automático, verificamos si cambió
                    ...
                    // Actualizamos el badge en la UI
    ...
    Promise.all(promises).then(() => {
        ...
        if (!manual && cambiosDetectados > 0) {
            // Utilizamos el toast general del sistema para dar una notificación visible
...
// Inicializar el polling (cada 30 segundos)
