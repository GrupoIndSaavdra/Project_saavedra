        ...
        for (; index < limit; index++) {
            ...
            Object.keys(pieza).forEach((key) => {
                if (key !== "colorPiece") {
                    ...
                    if (key === "btn_liberar") {
                        chunkHtml += `<td><a class="btn-liberar" href="#" onclick="event.preventDefault(); window.confirmarAccion('liberar', '${esc(pieza.workOrder)}', '${esc(pieza.noAssembly)}', '${esc(pieza.class)}', '${esc(pieza.process)}')"><img src="${window.liberar}" alt="Liberar" class="icono-liberar"></a></td>`;
                    } else if (key === "btn_rechazar") {
                        chunkHtml += `<td><a class="btn-liberar" href="#" onclick="event.preventDefault(); window.confirmarAccion('rechazar', '${esc(pieza.workOrder)}', '${esc(pieza.noAssembly)}', '${esc(pieza.class)}', '${esc(pieza.process)}')"><img src="${window.rechazar}" alt="Rechazar" class="icono-rechazar"></a></td>`;
...
function asignColorTr(status, error) {
    ...
    error = (error !== null && error !== undefined) ? String(error).trim() : "";

    function isGoodError(err) {
        let normalize = err.toLowerCase().trim();
        if (normalize === "") return true;
        
        let clean = normalize.replace(/ninguno/g, "").replace(/n\/a/g, "").replace(/[\s\/\,\-]/g, "");
        if (clean === "") return true; 
        
        if (/^\d{4}-\d{2}-\d{2}/.test(normalize)) return true;
        
        return false;
    }

    let goodCondition = isGoodError(error);

    if (status === 1) return "#79BFED"; 
    ...
    if (status === 5 || error.toLowerCase().includes("incompleto")) {
        return "#FFD700"; 
    }

    if (status === 3) return "#90EE90"; 
    if (status === 4) return "#DDA0DD"; 
...
function orderedArray(array) {
    return {
        ...
        btn_liberar: [array[9], array[5] ?? ""],
        btn_rechazar: array[9],
...
function createFilters() {
    ...
    if (Object.keys(window.selectedItems).length > 0) {
        document.querySelectorAll(".filters .input-filter, .filters .select-filter").forEach(el => {
...
function createStatusFilterUI() {
    ...
    document.querySelector(".filters").appendChild(divStatus);
}
