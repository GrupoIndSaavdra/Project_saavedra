...
function crearTabla(piezas, infoPiezas) {
    ...
    // release_pieces usa 'quality' como perfil por defecto
...
function updateStatusPersonFilter(statusValue) {
    ...
    // Liberadas/Rechazadas → liberador (celda índice 10 en release_pieces)
    ...
    rows.forEach(row => {
        ...
        if (useOperator) {
            ...
            if (!liberador) {
                // En release_pieces la columna "Liberado por" es la índice 10
    ...
    rows.forEach(row => {
        ...
        if (statusFilter !== "Todos" && statusPersonFilter && statusPersonFilter !== "Todos") {
            if (personUsesOperator) {
                ...
                if (!liberador) {
                    // En release_pieces la columna "Liberado por" es índice 10
                    let cells = row.querySelectorAll("td");
                    if (cells[10]) liberador = cells[10].textContent.trim();
                }
...
function setupGameFilterLogic() {
    // release_pieces usa "workOrder" y "class"
    const otSelect = document.querySelector('select[name="workOrder"]');
    const classSelect = document.querySelector('select[name="class"]');
