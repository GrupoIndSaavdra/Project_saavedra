...
function createTable() {
    ...
    let form = createForm("/process_production/storePiece");
    ...
    if (window.pieceToBeUsed) {
        ...
        if (window.arrayData["availableAssemblies"].length > 0) {
            if (window.arrayData["edit"] != 2) {
                ...
                form.action = window.baseUrl + "/process_production/selectAssembly";
...
function showInlinePasswordForm(type, imgElement = null) {
    ...
    form.action = window.baseUrl + "/process_production/verified";
        ...
        if (confirm("¿Estás seguro de que deseas terminar el reporte?")) {
            ...
            if (window.arrayData) {
                window.location.href =
                    window.baseUrl + "/process_production/finishReport/" + window.arrayData["meta"].id;
            } else {
                window.location.href = window.baseUrl + "/process_production/finishReport/0";
            }
        }
    });
}
...
function verifyQualityPasswordAjax(form, imgElement) {
    ...
    fetch(window.baseUrl + "/process_production/verifyQualityPassword", {
        method: "POST",
        headers: {
...
function showQualityReleaseModal(piecesData, qualityUserName = "") {
    ...
    // Crear formulario
    let form = createForm("/process_production/release_pieces");
 ...
 * Utiliza clases CSS .toastpremium definidas en process_production.css
 */
